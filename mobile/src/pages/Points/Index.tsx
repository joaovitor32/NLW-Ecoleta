...
const Points=()=>{
    ...
    useEffect(()=>{
        async function loadPosition(){
            ...
            if(status!=="granted"){
                Alert.alert('Problema','Precisamos de permissão para continuar');
                return;
            }
    ...
    <View style={styles.container}>
            ...
            <View style={styles.mapContainer}>
                {initialPosition[0]!==0&&initialPosition[1]!==0&&<MapView  style={styles.map} 
                ...
                {points.map(point=>(<Marker key={String(point.id)} onPress={()=>{handleNavigateToDetail(point.id)}} style={styles.mapMarker} coordinate={{latitude:point.latitude,longitude:point.longitude}}>
        ...
        <View style={styles.itemsContainer}> 
            ...
            {items.map(item=>(
                <TouchableOpacity key={String(item.id)} activeOpacity={0.6} style={[styles.item,selectedItems.includes(item.id)?styles.selectedItem:{}]} onPress={()=>{handleSelectedItem(item.id)}}>
                   <SvgUri width={42} height={32} uri={item.imageUrl}/>
